...
function doDelete(id){
	if(confirm("确定需要删除？")){
		location.href='gameCategory-delete.html?pageIndex=${pageIndex! }&ids=' + id;
	}
}
...
function deleteAll(){
	...
	if(i <= 0){
		...
		if(confirm("确定需要删除？")){
			...
			form1.action = "gameCategory-delete.html?pageIndex=${pageIndex! }";
			form1.submit();
		}
	}
}
...
<body>
    ...
    <div class="rightinfo"  style="overflow:auto;height:500px" id="contentDiv">
		<form action="gameCategory.html" method="post" name="form1" id="form1">
	    	<ul class="seachform">
	    		<li onclick="location.href='gameCategory-detail.html?pageIndex=${pageIndex! }'" class=".toolbar li"><span><img src="${staticPath! }/static/images/t01.png${staticVersion! }" /></span>新增</li>
	        	<li onclick="deleteAll()" class=".toolbar li"><span><img src="${staticPath! }/static/images/t03.png${staticVersion! }" /></span>删除</li>
	        	<li>
	        		<label>名称：</label><input name="entity.name" type="text" value="${(entity.name)! }" class="scinput-150" />
	        	</li>
		    ...
		    <table class="tablelist">
		        ...
		        <tr title="${item.name! }">
			        ...
			        <td>${item.name! }</td>
			        <td><#if item.status == '1'><font color="green">开启</font><#elseif item.status == '2'><font color="red">禁用</font></#if></td>
			        <td>${item.orderId! }</td>
			        <td>${(item.createTime?datetime)! }</td>
			        <td>
			        	<a href="gameCategory-detail.html?pageIndex=${pageIndex! }&id=${item.id! }" class="tablelink">修改</a> |
	    ...
	    <div class="pagin">
	        <@plugins.page action="gameCategory.html?pageIndex=" />
	    </div>
	</div>
