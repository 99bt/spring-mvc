...
function doDelete(id){
	if(confirm("确定需要删除？")){
		location.href='room-advertisement-delete.html?pageIndex=${pageIndex! }&name=${(name)! }&ids=' + id;
	}
}
...
function deleteAll(){
	...
	if(i <= 0){
		...
		if(confirm("确定需要删除？")){
			...
			form1.action = "room-advertisement-delete.html?pageIndex=${pageIndex! }";
			form1.submit();
		}
	}
}
...
<body>
    ...
    <div class="rightinfo"  style="overflow:auto;height:500px" id="contentDiv">
		<form action="advertisement.html" method="post" name="form1" id="form1">
	    	<ul class="seachform">
	    		<li onclick="location.href='room-advertisement-detail.html?pageIndex=${pageIndex! }&name=${(name)! }'" class=".toolbar li"><span><img src="${staticPath! }/static/images/t01.png${staticVersion! }" /></span>新增</li>
		    ...
		    <table class="tablelist">
		        ...
		        <tr title="${item.name! }">
			        ...
			        <td>${item.name! }</td>
			        <td><#if item.status == '0'><font color="red">下线</font><#elseif item.status == '1'><font color="green">在线</font></#if></td>
			        <td><#if item.type == '1'>房间广告</#if></td>
			        <td>${item.rate! }</td>
			        <td><@plugins.cutOff cutStr="${item.remark! }" cutLen="40" /></td>
			        <td>${(item.createTime?datetime)! }</td>
			        <td>
			        	<a href="room-advertisement-detail.html?pageIndex=${pageIndex! }&id=${item.id! }&name=${(name)! }" class="tablelink">修改</a> |
	    ...
	    <div class="pagin">
	        <@plugins.page action="advertisement.html?name=${name! }&pageIndex=" />
	    </div>
	</div>
