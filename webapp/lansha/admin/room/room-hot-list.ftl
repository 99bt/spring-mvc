...
function doDelete(id){
	if(confirm("确定需要删除？")){
		location.href='room-hot-delete.html?pageIndex=${pageIndex! }&ids=' + id;
	}
}
...
function deleteAll(){
	...
	if(i <= 0){
		...
		if(confirm("确定需要删除？")){
			...
			form1.action = "room-hot-delete.html?pageIndex=${pageIndex! }";
			form1.submit();
		}
	}
}
...
<body>
    ...
    <div class="formbody">
	    ...
	    <div class="itab">
	  	<ul> 
		    ...
		    <li><a href="#tab1" class="selected">热门房间</a></li>
    ...
    <div class="rightinfo"  style="overflow:auto;height:500px" id="contentDiv">
		<form action="room-hot.html" method="post" name="form1" id="form1">
	    	<ul class="seachform">
	    	<#--
		    ...
		    <table class="tablelist">
		        ...
		        <#list listMapRootHot?if_exists as item>
			        <tr title="${item.name! }">
				        ...
				        <td>
				        	<a href="room-hot-detail.html?id=${item.id! }&pageIndex=${pageIndex! }" class="tablelink">修改</a> |
	    ...
	    <div class="pagin">
	        <@plugins.page action="room-hot.html?pageIndex=" />
	    </div>
	</div>
    <script type="text/javascript">
