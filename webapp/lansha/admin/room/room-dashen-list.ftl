...
function doDelete(id){
	if(confirm("确定需要删除？")){
		location.href='room-dashen-delete.html?pageIndex=${pageIndex! }&ids=' + id;
	}
}
...
function deleteAll(){
	...
	if(i <= 0){
		...
		if(confirm("确定需要删除？")){
			...
			form1.action = "room-dashen-delete.html?pageIndex=${pageIndex! }";
			form1.submit();
		}
	}
}
...
<body>
    ...
    <div class="rightinfo"  style="overflow:auto;height:500px" id="contentDiv">
		    ...
		    <table class="tablelist">
		        ...
		        <#list listMapRootHot?if_exists as item>
			        <tr title="${item.name! }">
				        ...
				        <td>
				        	<a href="room-dashen-detail.html?id=${item.id! }&pageIndex=${pageIndex! }" class="tablelink">修改</a> |
	    ...
	    <div class="pagin">
	        <@plugins.page action="room-dashen.html?pageIndex=" />
	    </div>
	</div>
    <script type="text/javascript">
