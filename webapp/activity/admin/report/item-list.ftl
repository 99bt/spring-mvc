...
<body>
    ...
    <div class="formbody">
	    <div class="rightinfo" style="overflow:auto;height:500px" id="contentDiv">
			<form action="giftReport.html" method="post" name="form1" id="form1">
		    	<ul class="seachform">
		    		<li>
			        ...
			        <tbody>
				        <#list list?if_exists as item>
					        <tr title="${(item.name)! }">
						        ...
						        <td>
						        	<a href="giftReport-view.html?pageIndex=${pageIndex! }&name=${name! }&entity.itemId=${(item.id)! }" class="tablelink">查看报表</a>
								</td>
					        </tr>
				        </#list>
			        </tbody>
		    ...
		    <div class="pagin">
		        <@plugins.page action="giftReport.html?name=${name! }&pageIndex=" />
		    </div>
		</div>
    </div>
